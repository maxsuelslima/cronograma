...
export async function GET(
  ...
  if (querySnapshot.empty) {
    return new Response("Nenhum dado encontrado para este usuário", {
      status: 404,
    });
  }
...
export async function recuperarDadosUsuario(idUsuario: string): Promise<TopicosEstudados[]> {
    const nomeColecao = "topicos_estudados_usuario";
    const collectionRef = collection(firestore, nomeColecao);
    const q = query(
      collectionRef,
    ...
    const querySnapshot = await getDocs(q);
    if (querySnapshot.empty) {
      console.log("Nenhum dado encontrado para este usuário:", idUsuario);
      return [];
    }
    const dados = querySnapshot.docs[0].data();
    console.log("Dados do Firestore para o usuário:", idUsuario, dados);
    return dados.topicos_estudados || [];
}
export async function PATCH(request: NextRequest) {
  ...
  try {
    if (querySnapshot.empty) {
      ...
      await setDoc(
        doc(firestore, nomeColecao, docId),
        {
          id_usuario: idUsuario,
          topicos_estudados: topicos_estudados,
        },
    ...
    return new Response("Erro ao atualizar os tópicos estudados", {
      status: 500,
    });
  }
}
